...
class NaProducter:
    ...
    def __init__(self, seed_value=2022):
    ...
    def pick_epsilon(self, X, quant=0.5, mult=0.05, max_points=2000):
        ...
        idx = self.rng.choice(len(X_), min(max_points, len(X_)), replace=False)
        X = X_[idx]
    ...
    def MAR_mask(self, X, p, p_obs):
        ...
        coeffs = self.pick_coeffs(X, idxs_obs, idxs_nas)
        ### Pick the intercepts to have a desired amount of missing values
        intercepts = self.fit_intercepts(X[:, idxs_obs], coeffs, p)
    ...
    def MNAR_mask_logistic(self, X, p, p_params=.3, exclude_inputs=True):
        """
        ...
        d_params = max(int(p_params * d),
                       1) if exclude_inputs else d  ## number of variables used as inputs (at least 1)
        d_na = d - d_params if exclude_inputs else d  ## number of variables masked with the logistic model

        ### Sample variables that will be parameters for the logistic regression:
        idxs_params = self.rng.choice(d, d_params, replace=False) if exclude_inputs else np.arange(d)
        ...
        ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
        coeffs = self.pick_coeffs(X, idxs_params, idxs_nas)
        ### Pick the intercepts to have a desired amount of missing values
        intercepts = self.fit_intercepts(X[:, idxs_params], coeffs, p)
    ...
    def MNAR_self_mask_logistic(self, X, p):
        ...
        ### Pick the intercepts to have a desired amount of missing values
        intercepts = self.fit_intercepts(X, coeffs, p, self_mask=True)

        ps = torch.sigmoid(X * coeffs + intercepts)

        torch.manual_seed(self.seed_value)
        ber = torch.rand(n, d) if to_torch else self.rng.random((n, d))
        return ber < ps if to_torch else ber < ps.numpy()

    def MNAR_mask_quantiles(self, X, p, q, p_params, cut='both', MCAR=False):
        """
        ...
        idxs_na = self.rng.choice(d, d_na, replace=False)  ### select at least one variable with missing values
    ...
    def pick_coeffs(self, X, idxs_obs=None, idxs_nas=None, self_mask=False):
        n, d = X.shape
        if self_mask:
            torch.manual_seed(self.seed_value)
    ...
    def produce_NA(self, X, p_miss, mecha="MCAR", opt=None, p_obs=None, q=None):
        """
        ...
        if mecha == "MAR":
            mask = self.MAR_mask(X, p_miss, p_obs).double()
        elif mecha == "MNAR" and opt == "logistic":
            mask = self.MNAR_mask_logistic(X, p_miss, p_obs).double()
        elif mecha == "MNAR" and opt == "quantile":
            mask = self.MNAR_mask_quantiles(X, p_miss, q, 1 - p_obs).double()
        elif mecha == "MNAR" and opt == "selfmasked":
            mask = self.MNAR_self_mask_logistic(X, p_miss).double()
        else:
            torch.manual_seed(self.seed_value)
